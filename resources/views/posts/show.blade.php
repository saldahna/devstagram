...
@section('contenido')
    ...
    <div class="container mx-auto md:flex">
        ...
        <div class="md:w-1/2">
            ...
            <div>
                {{-- user viene de la relación de la clase Post, 
                    donde indica que un usuario puede tener múltiples
                    post, pero un post solo puede petenecer a un usuario --}}
            ...
            @auth
                @if ($post->user_id === auth()->user()->id)    
                    <form method="POST" action="{{ route('posts.destroy', $post) }}">
                        {{-- El método @method('DELETE') se conoce como Método Spoofing,
                             y se usa porque el action <form> no soporta el DELETE  --}}
                        @method('DELETE')
                        @csrf
                        <input 
                            type="submit" 
                            value="Eliminar publicación"
                            class="bg-red-500 hover:bg-red-600 p-2 text-white rounded-lg font-bold mt-4 cursor-pointer" 
                        />
                    </form>
                @endif
            @endauth
        ...
        <div class="md:w-1/2 p-5">
            ...
            <div class="shadow bg-white p-5 mb-5 rounded-lg">
                ...
                <form action="{{ route('comentarios.store', ['post' => $post, 'user' => $user]) }}" method="post">
                    @csrf
                    <div class="mb-5">
                        <textarea name="comentario" id="comentario" class="border p-3 w-full rounded-lg @error('name') border-red-500 @enderror" cols="30" rows="5" placeholder="Comenta esta publicación"></textarea>
                        @error('comentario')
